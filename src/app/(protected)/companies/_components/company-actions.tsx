...
export function CompanyActions({ company, onEdit }: CompanyActionsProps) {
  ...
  const handleNavigateToGrowRooms = () => {
    if (company.id) {
      router.push(AppRoutes.COMPANIES_GROW_ROOMS(company.id));
    }
  };

  const handleNavigateToUsers = () => {
    if (company.id) {
      router.push(`${AppRoutes.USERS}?companyId=${company.id}`);
    }
  };
    ...
    <AlertDialog>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={handleNavigateToGrowRooms}>
